...
import "./PUBGPlayersPage.css";
import axios from "axios";
import mlbbBanner from "../../assets/images/pubgmnew.png";

const PUBGPlayersPage = ({ theme }) => {
  ...
  useEffect(() => {
    if (selectedRegion === "All") {
      axios
        .get("http://localhost:8080/pubg_players")
        .then((res) => {
      ...
      axios
        .get(`http://localhost:8080/pubg_players/region/${selectedRegion}`)
        .then((res) => {
          const sorted = [...res.data].sort((a,b) => a.playerCountry.localeCompare(b.playerCountry));
          setPlayersData(sorted);
        });
  ...
  return (
    <div className={`teams-page ${theme}`}>
        ...
        <nav className="nav-tabs">
          {["SEA", "South Asia", "MEA", "Americas", "China", "Korea", "Japan", "India", "All"].map(
            (region) => (

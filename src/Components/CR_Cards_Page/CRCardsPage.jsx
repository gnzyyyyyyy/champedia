...
const CRCardsPage = ({ theme }) => {
  ...
  useEffect(() => {
    if (selectedType === "All") {
      axios
        .get("http://localhost:8080/cr_cards")
        .then((res) => {
      ...
      axios
        .get(`http://localhost:8080/cr_cards/type/${selectedType}`)
        .then((res) => {
            const sortedCards = res.data.sort(
            (a, b) => rarityOrder[a.cardRarity] - rarityOrder[b.cardRarity]
            );

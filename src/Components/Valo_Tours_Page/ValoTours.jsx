...
const VALOTour = ({theme}) => {
    ...
    useEffect (() => {
        if (selectedTier === "All") {
            axios
            .get("http://localhost:8080/valo_tours")
            .then((res) => setTourData(res.data));
        } else {
            axios
            .get(`http://localhost:8080/valo_tours/tier/${selectedTier}`)
            .then((res) => setTourData(res.data));
        }
    }, [selectedTier]);

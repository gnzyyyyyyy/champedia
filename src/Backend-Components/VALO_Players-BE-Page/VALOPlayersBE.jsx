...
const MLBBPlayersBE = ({theme}) => {
    ...
    useEffect(() => {
        if(selectedRegion === "All") {
            axios.get("http://localhost:8080/valo_players")
                .then(res => setPlayers(res.data))
        } else {
            axios.get(`http://localhost:8080/valo_players/region/${selectedRegion}`)
                .then(res => setPlayers(res.data))
        }
    }, [selectedRegion]);

    useEffect(() => {
        axios.get("http://localhost:8080/valo_teams")
    ...
    const handleAddPlayer = () => {
        axios
            .post("http://localhost:8080/valo_players", newPlayer)
            .then(() => {
                setShowForm(false);
                resetForm();
    ...
    const handleUpdatePlayer = () => {
        axios
            .put(`http://localhost:8080/valo_players/${editPlayerID}`, newPlayer)
            .then(() => {
                setShowForm(false);
                resetForm();
                window.location.reload();
    ...
    const confirmRemove = () => {
        axios
            .delete(`http://localhost:8080/valo_players/${playerToDelete}`)
            .then(() => {
    ...
    return (
        <div className={`players-page ${theme}` }>
            ...
            <header className="header">
                ...
                {["APAC", "EMEA", "AMER", "China", "All"].map((region) => (

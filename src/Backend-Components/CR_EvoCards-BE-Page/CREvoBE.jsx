...
const CREvoBE = ({ theme }) => {
    ...
    useEffect(() => {
        if (selectedType === "All") {
            axios.get("http://localhost:8080/cr_evo")
                .then(res => setCards(res.data));
        } else {
            axios.get(`http://localhost:8080/cr_evo/type/${selectedType}`)
                .then(res => setCards(res.data));
        }
    }, [selectedType]);
    ...
    // Add card
    const handleAddCard = () => {
        axios.post("http://localhost:8080/cr_evo", newCard)
            .then(() => {
                setShowForm(false);
                resetForm();
    ...
    const handleUpdateCard = () => {
        axios.put(`http://localhost:8080/cr_evo/${editCardID}`, newCard)
            .then(() => {
                setShowForm(false);
                resetForm();
                window.location.reload();
    ...
    // Confirm remove
    const confirmRemove = () => {
        axios.delete(`http://localhost:8080/cr_evo/${cardToDelete}`)
            .then(() => {

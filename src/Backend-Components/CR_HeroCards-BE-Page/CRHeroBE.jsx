...
const CRHeroBE = ({ theme }) => {
    ...
    useEffect(() => {
        if (selectedType === "All") {
            axios.get("http://localhost:8080/cr_hero")
                .then(res => setCards(res.data));
        } else {
            axios.get(`http://localhost:8080/cr_hero/type/${selectedType}`)
                .then(res => setCards(res.data));
        }
    }, [selectedType]);
    ...
    // Add card
    const handleAddCard = () => {
        axios.post("http://localhost:8080/cr_hero", newCard)
            .then(() => {
                setShowForm(false);
                resetForm();
    ...
    const handleUpdateCard = () => {
        axios.put(`http://localhost:8080/cr_hero/${editCardID}`, newCard)
            .then(() => {
                setShowForm(false);
                resetForm();
                window.location.reload();
    ...
    // Confirm remove
    const confirmRemove = () => {
        axios.delete(`http://localhost:8080/cr_hero/${cardToDelete}`)
            .then(() => {

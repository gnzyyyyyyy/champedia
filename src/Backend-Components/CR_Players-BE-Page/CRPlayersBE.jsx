...
import "./CRPlayersBE.css";
import crLogo from "../../assets/images/cr_logo.png";

const CRPlayersBE = ({theme}) => {
    ...
    useEffect(() => {
        if(selectedRegion === "All") {
            axios.get("http://localhost:8080/cr_players")
                .then(res => setPlayers(res.data))
        } else {
            axios.get(`http://localhost:8080/cr_players/region/${selectedRegion}`)
                .then(res => setPlayers(res.data))
        }
    }, [selectedRegion]);

    useEffect(() => {
        axios.get("http://localhost:8080/cr_teams")
    ...
    const handleAddPlayer = () => {
        axios
            .post("http://localhost:8080/cr_players", newPlayer)
            .then(() => {
                setShowForm(false);
                resetForm();
    ...
    const handleUpdatePlayer = () => {
        axios
            .put(`http://localhost:8080/cr_players/${editPlayerID}`, newPlayer)
            .then(() => {
                setShowForm(false);
                resetForm();
                window.location.reload();
    ...
    const confirmRemove = () => {
        axios
            .delete(`http://localhost:8080/cr_players/${playerToDelete}`)
            .then(() => {
        ...
        <div className={`players-page ${theme}` }>
            ...
            {showForm && (
                ...
                <div className="modal-content">
                    ...
                    <div className="form-group">
                    <label>Region:</label>
                    <input name="playerRegion" value={newPlayer.playerRegion} onChange={handleChange}/>
                    </div>

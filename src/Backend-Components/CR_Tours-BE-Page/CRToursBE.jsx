...
import "./CRToursBE.css";
import crLogo from "../../assets/images/cr_logo.png";

const CRToursBE = ({ theme }) => {
    ...
    useEffect(() => {
        if(selectedTier === "All"){
            axios.get("http://localhost:8080/cr_tours")
                .then(res => setTournaments(res.data))
        } else {
            axios.get(`http://localhost:8080/cr_tours/tier/${selectedTier}`)
                .then(res => setTournaments(res.data))
        }
    }, [selectedTier]);
    ...
    const handleAddTournament = () => {
        axios.post("http://localhost:8080/cr_tours", newTournament)
            .then(() => {
                setShowForm(false);
                resetForm();
    ...
    const handleUpdateTournament = () => {
        axios.put(`http://localhost:8080/cr_tours/${editTournamentID}`, newTournament)
            .then(() => {
                setShowForm(false);
                resetForm();
                window.location.reload();
    ...
    const confirmRemove = () => {
        axios.delete(`http://localhost:8080/cr_tours/${tournamentToDelete}`)
            .then(() => {
